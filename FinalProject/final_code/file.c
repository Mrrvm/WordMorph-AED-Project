...
void manage_pal_data(char *word1, char *word2, vector *indexing_vector, int typeof_exe) {
	
	int word_len = 0, i = 0, diff = 0;
	element *got_element = NULL;
	
	/*Gets the element with the word length from the indexing_vector*/
	word_len = strlen(word1);
	...
	if(got_element == NULL) {
		/*Creates the element with 0 words and the number of mutations
			of the current problem, if the difference of letters is equal
			or minor to 1, sets the number of mutations of the problem as 1*/
		if(diff <= 1)
			got_element = create_element(0, diff);
		else
			got_element = create_element(0, typeof_exe); 
		set_item_to_vector(word_len, indexing_vector, (element *)got_element);
	}
	else {
		/*If the difference is letter is bigger than 1*/
		if(diff > 1) {
	...
	while(fscanf(pal_file, "%99s", first_word) == 1) {
		...
		if(fscanf(pal_file, "%99s", second_word) == 1) {
			if(fscanf(pal_file, "%d", &typeof_exe) == 1) {
				manage_pal_data(first_word, second_word, indexing_vector, typeof_exe);
			}
		}
	}
	fclose(pal_file);
...
void write_to_file(vector *indexing_vector, pal_problem *new_problem, FILE *output_file, path_element *path_vector) {
	...
	if(path_vector != NULL) {
		...
		word1 = get_word_vector_word(src_index, word_vector);
    	word2 = get_word_vector_word(dest_index, word_vector);
    	for(i=0; i < word_len; i++) {
        	if(word1[i] != word2[i])
           		diff++;
    	}
